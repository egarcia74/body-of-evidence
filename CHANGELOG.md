...
- Signature envelope specified (detached `.sig` over the content digest, so re-serialisation cannot invalidate it) and identified as the trust anchor the hash chain needs; implementation deferred to phase 4; resolves the standing contradiction between `DECISIONS.md` ("must be designed together with signing") and `ROADMAP.md` ("Unprioritised / Future")

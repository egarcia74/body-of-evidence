...
- `boe_files` traversal (`find_entity_files`, `find_all_symlinks`) is now a single shared `os.walk(onerror=...)`-based walk; a new `find_traversal_errors` surfaces any subdirectory that could not be listed, and `run_reference_validation` turns that into a failing `PACKAGE_SUBTREE_UNREADABLE` diagnostic instead of silently certifying a package part of which was never inspected (M-22)

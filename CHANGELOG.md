...
- **A human must accept any ADR that changes what a release is, what "verified" means, what a reference resolves to, what identity is, or what the platform claims to guarantee.** `AI_GUIDELINES.md` exempted "schema and tooling" from its AI limits; that exemption was written when platform work meant validation scripts, and D-016 sat inside it while deciding what a release *is*. The right thing happened anyway — options were put to the sponsor and the sponsor chose — but nothing required it. The rule attaches to the *consequence* rather than to the tooling/content boundary, which thins as the platform grows
...
- **Two of the new issue form's own required fields were vacuously required.** "Primary sources (at least 3)" and "Initial claims (2-3)" were single textareas — a textarea cannot enforce a count, so those minimums were labels rather than floors — and both were PREFILLED with `value: "1.\n2."`, which makes a field non-empty before the submitter types anything, satisfying `required: true` with the numbering alone. The mechanism the whole markdown→issue-form conversion was about did not hold for them. Each source and each claim now has its own required field, which is the only shape in which the count is a floor

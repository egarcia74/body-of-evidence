...
**What was decided, by whom** (this ADR is bound by its own rule, so it states this rather than implying it): the sponsor was asked whether AI-drafted architecture decisions should require human sign-off, given that `AI_GUIDELINES.md` exempts "schema and tooling" and D-016 fell inside that exemption while deciding what a release is. The sponsor accepted the rule as proposed. The sponsor did NOT decide the wording, which is mine, nor the privacy-gate change recorded below, which is also mine and is marked as such.

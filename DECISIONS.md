...
- **No `.coveragerc`.** `--cov=scripts` already scopes measurement to the `scripts/` package; there is no subpackage or generated-code directory under `scripts/` that needs excluding, and `coverage.py` does not measure `__pycache__` bytecode regardless. A separate config file would duplicate what the CLI flag already states.
- **`.gitignore`** gained `coverage.xml` alongside the pre-existing `.coverage` entry — both are generated, not source.

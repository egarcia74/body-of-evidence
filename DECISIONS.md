...
- **The validate→publish digest chain closes content TOCTOU (D-027's IOU).** Validation already reads each document's bytes exactly once and records a SHA-256 (`DiscoveredDocument.digest`). Compiling an Edition writes those same digests into `members[].digest`. Publication re-reads each file, recomputes, and refuses to publish on any mismatch. "What was validated" and "what was published" therefore become provably the same bytes. Note precisely what this does and does not give: it detects a file changing between validation and publication; it does not prevent the change, and it says nothing about the filesystem after publication. That is the honest scope, and it is a genuine closure of the gap D-027 documented rather than a restatement of it.

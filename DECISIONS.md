...
- **Edition body.** `edition_format`, `edition_id`, `parent_edition_id` + `parent_content_digest` (both null for the first Edition), `package_slug`, `investigation_id`, `release_version`, `schema_version`, `methodology_version`, `released_at`, `members[]`, `imports[]`, `notes`. Each member is `{id, version_id, path, digest}` — the stable id, the exact version, the path, and the SHA-256 of that file's bytes. Members are ordered by `id` for determinism. `content_digest` is deliberately NOT a field inside the body (a document cannot contain its own digest); it is computed on read and carried by whoever *points at* the Edition.

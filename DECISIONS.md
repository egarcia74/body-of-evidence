...
- **`CLAUDE.md`**'s Commands section gained one line showing the local coverage invocation (`python3 -m pytest tests/ --cov=scripts --cov-report=term-missing`), alongside the existing plain `pytest -q` line rather than replacing it.

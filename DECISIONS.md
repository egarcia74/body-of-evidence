...
**Still open after this decision:** Deciding an actual minimum coverage bar (CI `--cov-fail-under` and/or SonarQube Cloud quality-gate coverage condition) once a baseline number exists and has been reviewed. Promoting the SonarQube quality gate to a required check overall remains D-022's still-open item, now with one more of its inputs (coverage) actually present.

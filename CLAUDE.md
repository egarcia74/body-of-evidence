...
- Three independent reviews in `docs/reviews/` (3.5 → 4.8 → 5.8/10). Third pass independently verified the suite runs green and confirmed C-01 fixed; its two probe findings (revision endpoints of unrelated entities accepted; manifest without its Investigation accepted) are FIXED per D-017, plus symlink-escape containment.
- All ADRs in `DECISIONS.md` (D-001..D-017). D-016 is the accepted DIRECTION for immutable edition manifests — design ADR still required; do not implement piecemeal.
...
9. **Revisions connect versions of ONE entity.** Endpoint existence is not enough — validators check endpoint identity and type, and that superseded versions are not current in the manifest.
10. **Validation must never be vacuous.** `validate.py` fails on empty runs; fixtures prove both directions with exact-error assertions.
...
python3 -m pytest tests/ -q               # 23 tests, fixture-driven, exact-error assertions

...
- Six independent reviews in `docs/reviews/` (3.5 → 4.8 → 5.8 → 6.2 → 6.4 → 6.6/10). Each pass treats the previous round's remediation claims as propositions to falsify, not evidence of completion — and EVERY pass so far has found at least one claim that was narrower than stated, including the fix for the previous pass's narrowness. Sixth pass, on D-019's fixes: the package-scoping fix used a lossy single-owner id index that produced a false positive in D-019's OWN test fixture (H-17); the reference-check dispatch was a hand-written if/elif that silently omitted 5 of ~30 schema-declared reference fields (H-18, fixed with a declarative registry PLUS a completeness test that scans schemas directly — the test found a 6th omitted field none of the six reviews had caught); and the symlink fix covered package roots and manifest-listed paths but not an ordinary unmanifested entity-file symlink, which could read outside the package or crash on a dangling target (H-19). All FIXED per D-020. **Pattern to internalise:** "I fixed the specific case the reviewer demonstrated" is not the same as "I fixed the general claim" — before reporting any fix as done, ask what OTHER inputs would exercise the same code path, not just the one in the review's probe.
- All ADRs in `DECISIONS.md` (D-001..D-020). D-016 is the accepted DIRECTION for immutable edition manifests — design ADR still required; do not implement piecemeal.
...
9. **Every reference is package-scoped by default** — every field in `validate_references.py`'s `REFERENCE_FIELDS`/`NESTED_REFERENCE_FIELDS` registry (D-020/H-18; this IS the complete list, enforced by a schema-scanning completeness test — do not add a reference field to a schema without adding it here too), not just Revision endpoints (D-018/H-02b). A stable id CAN legitimately appear in more than one file — `id_index` is a multimap; a reference prefers a same-package match and only flags `REF_WRONG_PACKAGE` when none exists (D-020/H-17; a naive single-owner index produces false positives). There is deliberately no way yet to declare "package A depends on package B". A symlink is rejected everywhere — a package ROOT (D-019/H-15), a manifest-listed entity path (D-018/M-11), AND an ordinary unmanifested entity file (D-020/H-19, since historical/superseded versions are legitimately unmanifested by design) — before any file is ever read.
...
python3 -m pytest tests/ -q               # 44 tests, fixture-driven, exact diagnostic assertions incl. CLI-level integration + schema-completeness scan
python3 scripts/validate.py --root <dir>  # validate an arbitrary package directory instead of <repo>/investigations (D-019/M-15; --root is validated before use — D-020/M-18)
